...
def build_demo():
    with gr.Blocks(title="光谱分析与优化") as demo:
        ...
        with gr.Tab("2. 优化进度"):
            gr.Markdown("### 优化进度")
            progress_bar = gr.Slider(label="进度", minimum=0, maximum=100, value=0, interactive=False)
            log_box = gr.Textbox(label="日志", lines=10, interactive=False)

        with gr.Tab("3. 结果查看"):
            gr.Markdown("### 结果查看")
            result_table = gr.Dataframe(headers=["Param", "Value"], datatype=["str", "number"])
            result_plot = gr.Plot()
